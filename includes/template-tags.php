...
function posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'af-theme' ),
...
function posted_by() {
	...
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'af-theme' ),
...
function entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'af-theme' ) );
		if ( $categories_list ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'af-theme' ) . '</span>', $categories_list );
		}

		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'af-theme' ) );

		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'af-theme' ) . '</span>', $tags_list );
		}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'af-theme' ),
					[
						'span' => [
							'class' => [],
						],
	...
	edit_post_link(
		sprintf(
			wp_kses(
				__( ' Edit <span class="screen-reader-text">%s</span>', 'af-theme' ),
				[
					'span' => [
						'class' => [],
					],
